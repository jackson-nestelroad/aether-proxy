...
// Base attributes and methods for managing data that may be collected from an input buffer or stream in one or more
// method calls.
class base_segment {
 public:
  ...
  // Commits all currently buffered data to be exported and marks the segment as complete.
  void commit();

  void mark_as_incomplete();
  void mark_as_complete();

  constexpr std::string_view string_view() const noexcept { return buffer_.string_view(); }

  inline const_buffer committed_data() const noexcept { return buffer_.data(); }

  inline streambuf& internal_buffer() noexcept { return buffer_; }
 ...
 protected:
  ...
  std::size_t num_bytes_read_last_;
