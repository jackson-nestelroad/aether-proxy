...
template <const std::string_view& S1, std::size_t... I1, const std::string_view& S2, std::size_t... I2>
...
constexpr char to_lowercase_func(char c) { return c >= 65 && c <= 90 ? c ^ 0x20 : c; }

// Convert an entire string to lowercase (one character at a time).
template <const std::string_view& S, std::size_t... I>
...
struct join<S1, S2> {
  static constexpr std::string_view value =
      string_view_detail::concat<S1, std::make_index_sequence<S1.size()>, S2, std::make_index_sequence<S2.size()>>::value;
};

// String joining recursive case.
template <const std::string_view& S, const std::string_view&... Rest>
struct join<S, Rest...> {
  static constexpr std::string_view value = join<S, join<Rest...>::value>::value;
};
...
struct to_lowercase : string_view_detail::to_lowercase<S, std::make_index_sequence<S.size()>> {};
