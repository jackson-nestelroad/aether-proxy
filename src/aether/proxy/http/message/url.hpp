...
struct url {
  ...
  enum class target_form { origin, absolute, authority, asterisk };
  ...
  // Builds the origin representation of the URL, which is everything except the path and search.
  ...
  constexpr port_t port_or_default(port_t def = 0) const { return netloc.port.value_or(def); }
