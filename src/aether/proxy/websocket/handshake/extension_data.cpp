...
result<extension_data> extension_data::from_header_value(std::string_view header) {
  std::size_t multi_extensions = header.find_first_of(extension_delim);
  if (multi_extensions != std::string::npos) {
    return error::websocket::invalid_extension_string("Multiple extensions present in single string");
  }

  const auto& parsed = util::string::split_trim<std::string>(header, param_delim);
  std::size_t size = parsed.size();
  if (size < 1) {
    return error::websocket::invalid_extension_string("No extension name found");
  }

  extension_data ext(parsed[0]);
  for (std::size_t i = 1; i < size; ++i) {
    ...
    if (assign_pos == std::string::npos) {
      ...
      ext.set_param(curr.substr(0, assign_pos), curr.substr(assign_pos + 1));
    }
  }
