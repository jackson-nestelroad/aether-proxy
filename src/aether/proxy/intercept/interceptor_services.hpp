...
#include "aether/proxy/intercept/base_interceptor_service.hpp"
...
#define WEBSOCKET_MESSAGE_EVENTS(X, other1, other2) X(received, 15, other1, other2)
...
#define CREATE_EVENT_ENUMS(name, macro_name, foreach) enum class name##_event{macro_name##_EVENTS(foreach, , )};
...
#define CREATE_ALL_EVENT_SFINAES(name, macro_name, foreach) \
  macro_name##_EVENTS(foreach, name, macro_name##_TYPES(DECLARE_VAL))
#define CREATE_EVENT_SFINAE(name, num, category, ...)                                                     \
  template <typename T, typename = void>                                                                  \
  struct has_on_##category##_##name : std::false_type {};                                                 \
  template <typename T>                                                                                   \
  struct has_on_##category##_##name<T, decltype(std::declval<T*>()->on_##category##_##name(__VA_ARGS__))> \
      : std::true_type {};
...
#define CREATE_VIRTUAL_INTERCECPTORS(name, macro_name, foreach) \
  macro_name##_EVENTS(foreach, name, macro_name##_TYPES(GIVE_BACK_TYPE))
#define CREATE_VIRTUAL_INTERCEPTOR(name, num, category, ...) \
  virtual void on_##category##_##name(__VA_ARGS__) {}
...
#undef GIVE_BACK_TYPE
 ...
 public:
  template <typename T>
  constexpr std::enable_if_t<interceptor_detail::is_event_enum_v<T>, interceptor_id&> operator[](T event) {
...
#define CREATE_INTERCEPTOR_SERVICES(name, macro_name, foreach)                              \
  class name##_interceptor_service                                                          \
   ...
   public:                                                                                  \
    template <typename T>                                                                   \
    std::enable_if_t<std::is_base_of_v<interceptor_hub, T>, void> attach_hub(T& hub) {      \
      macro_name##_EVENTS(foreach, name, )                                                  \
    }                                                                                       \
  };
#define ADD_INTERCEPTOR_IF_METHOD_EXISTS(name, num, category, other)                                     \
  if constexpr (interceptor_detail::has_on_##category##_##name<T>::value) {                              \
    hub[category##_event::name] =                                                                        \
        attach(category##_event::name, [&hub](auto&&... args) { hub.on_##category##_##name(args...); }); \
  }
...
#define ATTACH_HUB_LOCALLY(name, macro_name, foreach)               \
  if constexpr (interceptor_detail::has_some_##name##_event_v<T>) { \
    name.attach_hub(hub);                                           \
  }
