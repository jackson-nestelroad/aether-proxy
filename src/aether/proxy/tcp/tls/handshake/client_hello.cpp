...
client_hello client_hello::from_raw_data(const const_buffer& raw_data) {
  try {
    ...
    std::size_t handshake_length =
        util::bytes::concat(result.handshake_header[1], result.handshake_header[2], result.handshake_header[3]);
      ...
      while (index < data_length) {
        ...
        if (type == static_cast<std::size_t>(extension_type::server_name)) {
          ...
          while (index < prev_index + length) {
            read_byte_string(raw_data, index, 2);
            server_name entry;
          ...
          auto pair = result.extensions.emplace(static_cast<extension_type>(type), byte_array_t{});
          if (!pair.second) {
