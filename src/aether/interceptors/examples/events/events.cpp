...
void on_server_connect(connection_flow& flow) {
  out::safe_console::stream("Connected to ", flow.server.host(), " (", flow.server.endpoint(), ")\n");
}

void on_server_disconnect(connection_flow& flow) {
  out::safe_console::stream("Disconected from ", flow.server.host(), " (", flow.server.endpoint(), ")\n");
}

void on_http_request(connection_flow& flow, http::exchange& exch) {
  http::request& req = exch.request();
  out::safe_console::stream(req.method(), " request to ", req.target().absolute_string(), '\n');
}

void on_http_connect(connection_flow& flow, http::exchange& exch) {
  http::request& req = exch.request();
  out::safe_console::stream("CONNECT request to ", req.target().absolute_string(), '\n');
}
...
void on_http_websocket_handshake(connection_flow& flow, http::exchange& exch) {
  http::request& req = exch.request();
  out::safe_console::stream("WebSocket handshake request to ", req.target().absolute_string(), '\n');
}

void on_http_response(connection_flow& flow, http::exchange& exch) {
  http::response& res = exch.response();
  out::safe_console::stream(res.status(), " response from ", exch.request().target().absolute_string(), '\n');
}
...
void on_tunnel_start(connection_flow& flow) {
  out::safe_console::stream("TCP tunnel initiated with ", flow.server.host(), " (", flow.server.endpoint(), ")\n");
}

void on_tunnel_stop(connection_flow& flow) {
  out::safe_console::stream("TCP tunnel finished with ", flow.server.host(), " (", flow.server.endpoint(), ")\n");
}

void on_tls_established(connection_flow& flow) {
  out::safe_console::stream("TLS established with ", flow.server.host(), " (", flow.server.endpoint(), ")\n");
}

void on_tls_error(connection_flow& flow) { out::safe_error::stream("TLS error: ", flow.error, '\n'); }
...
void on_websocket_start(connection_flow& flow, websocket::pipeline& pline) {
  out::safe_console::stream("WebSocket connection established with ", flow.server.host(), " (", flow.server.endpoint(),
                            ")\n");
}

void on_websocket_stop(connection_flow& flow, websocket::pipeline& pline) {
  out::safe_console::stream("WebSocket connection finished with ", flow.server.host(), " (", flow.server.endpoint(),
                            ") (Close code = ", pline.close_code(), ")\n");
}
...
void on_websocket_message_received(connection_flow& flow, websocket::pipeline& pline, websocket::message& msg) {
  out::safe_console::stream("WebSocket message received from ", msg.origin(), '\n');
}
...
void attach_events(proxy::server& server) {
  ...
  server.interceptors().websocket_message.attach(intercept::websocket_message_event::received,
                                                 on_websocket_message_received);
}
