...
namespace interceptors::examples {
    void pokengine_interceptor::on_http_connect(connection_flow &flow, http::exchange &exch) {
        if (exch.request().get_target().is_host(host_name, port)) {
    ...
    void pokengine_interceptor::on_websocket_message_received(connection_flow &flow, websocket::pipeline &pline, websocket::message &msg) {
        if (flow.server.is_connected_to(host_name, port)) {
            out::safe_console::stream(msg.size(), " bytes received from the ", msg.get_origin(), '\n');
        }
    }
}